...
from ckanext.hack4laatd.logic import schema
from ckanext.hack4laatd.model import Event, VolunteeringOpportunity
from ckanext.hack4laatd import tagging
...
def event_create(context, data_dict):
    '''Create an Event'''
    toolkit.check_access('ckanext_hack4laatd_event_create', context, data_dict)
...
def volunteering_create(context, data_dict):
    ...
    toolkit.check_access('ckanext_hack4laatd_volunteering_create',
                         context, data_dict)
...
def event_delete(context, data_dict):
    '''Delete an Event'''
    toolkit.check_access('ckanext_hack4laatd_event_delete', context, data_dict)
...
def volunteering_delete(context, data_dict):
    ...
    toolkit.check_access('ckanext_hack4laatd_volunteering_delete',
                         context, data_dict)
...
def event_show(context, data_dict):
    '''Show an Event'''
    toolkit.check_access('ckanext_hack4laatd_event_show', context, data_dict)
...
def volunteering_show(context, data_dict):
    ...
    toolkit.check_access('ckanext_hack4laatd_volunteering_show',
                         context, data_dict)
...
def volunteering_list(context, data_dict):
    ...
    toolkit.check_access('ckanext_hack4laatd_volunteering_show', context,
                         data_dict)
...
def config_option_update(context, data_dict):
    ...
    if 'ckanext.hack4laatd.featured_image' in data_dict:
        ...
        value = data_dict['ckanext.hack4laatd.featured_image']
        if value \
           and not value.startswith('http') \
           and not value.startswith('/'):
            image_path = 'uploads/admin/'
            value = h.url_for_static('{0}{1}'.format(image_path, value))
        data_dict['ckanext.hack4laatd.featured_image'] = value
