...
def get_featured_image_url(default):
    return config.get('ckanext.hack4laatd.featured_image') or default


def get_editable_region(name):
    try:
        regions = json.loads(config.get('ckanext.hack4laatd.editable_regions', '{}'))
        return regions[name]
    except Exception:
        return ''
...
def get_minimum_views_for_trending():
    return int(config.get('ckanext.hack4laatd.trending_min') or '10') or 10
